...
// Disease detection validation (for ESP32-CAM)
export const validateDiseaseDetection = [
  ...
  body('detectedDisease')
    .isString()
    .trim()
    .isLength({ min: 1, max: 50 })
    .withMessage('Detected disease must be a non-empty string'),
  ...
  body('growthStage')
    .optional()
    .isIn(['seedling', 'vegetative', 'flowering', 'fruiting', 'mature', 'unknown'])
    .withMessage('Growth stage must be one of: seedling, vegetative, flowering, fruiting, mature, unknown'),
  ...
  body('edgeImpulseData')
    .optional()
    .custom((value) => {
      if (typeof value !== 'object') {
        throw new Error('edgeImpulseData must be an object');
      }
      return true;
    }),
  ...
  body('plantHealth')
    ...
    .custom((value) => {
      if (typeof value !== 'object') {
        throw new Error('plantHealth must be an object');
      }
      return true;
    }),
