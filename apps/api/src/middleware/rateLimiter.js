...
function isAdminIp(req) {
  ...
  return config.ADMIN_IPS.includes(clientIp);
}

function isActiveAdminRequest(req) {
  try {
    // Check against admin IP whitelist first
    if (isAdminIp(req)) {
      return true;
    }

    const originalUrl = String(req.originalUrl || '');
    if (originalUrl.includes('/api/config/admin-logs')) {
      return true;
    }
    ...
    return payload?.role === 'admin' && payload?.status === 'active';
  } catch {
    return false;
  }
}
